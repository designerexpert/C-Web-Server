...
void get_date(int fd)
{
  ...
  sprintf(response_body, "GMT Time is: %2d:%02d\nDate is: %d/%d/%d", (info->tm_hour) % 24, info->tm_min, info->tm_mday, info->tm_mon, info->tm_year + 1900);
...
void handle_http_request(int fd)
{
  ...
  if (strcmp(request_type, "GET") == 0) // Handle GET Request
  {
